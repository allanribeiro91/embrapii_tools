...
# U.name = Unidade Embrapii
# code = código do projeto
# P.status = status do projeto ('Em andamento' 'Concluído' 'Atrasado' 'Cancelado' 'Encerrado' 'Desqualificado' 'Suspenso') - retirar desqualificados??
# Periodo = período da prestação de contas? Bagunçado, não usar!!
# SitParecer = situação do parecer ('Em análise' 'Fechado', 'Aberto')
# month_year = mês e ano (AAAA-MM-DD)
# total_hours = horas de trabalho
# value = valor pago
# availability = disponibilidade (horas/mês)
# start_date = data de início
# end_date = data de fim (possui valores faltantes - significa ativo?)
# cpf = cpf do pesquisador
# PE.name = nome do pesquisador
